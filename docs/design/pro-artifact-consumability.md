...
### E2E result (cluster `ecp-us1`, us-east-1, 2026-09-03)

Images built and pushed to ECR at `1.0.16-dev-jandex`; chart generated by
`build-local.sh --helm` and installed fresh into `kube-microvm`.

| Check | Result |
|---|---|
| Operator pod | `1/1 Running`, no errors |
| Reconcilers registered and started | 4 — `microvmreconciler`, `microvmimagereconciler`, `microvmreplicaset-reconciler`, `microvmnetwork-reconciler` |
| `app.envs.MICROVM_AUTH_AGENT_IMAGE` in chart | `<ecr>/codriverlabs/kube-microvm-auth-agent:1.0.16-dev-jandex` |
| Same value on the operator Deployment | matches |
| **Injected sidecar image** | `<ecr>/codriverlabs/kube-microvm-auth-agent:1.0.16-dev-jandex` — **the fix; previously `:latest`** |
| Sidecar pulls and runs | pod `2/2 Running`, agent started and polling for its MicroVM |

The quota finding from §1.2 B2 is also visible live in the operator's startup log,
independently of the chart:

```
QuotaDiscovery: Quota discovery disabled — using configured values:
  run=5/s terminate=10/s suspend=2/s authToken=50/s imageBuilds=10
```

These are the code defaults sitting exactly at the AWS burst limits, confirming the
one-below values in the removed mappings never applied.
